...
clean_df <- function(df){
  ...
  dot <- "\\."
  counts_name <- str_split(counts_name, pattern = dot)[[1]][6]
...
#test<-read_data("/usr4/bf527/dlenci/Documents/project-3-saxophone-1/counts_data")
...
box_plot <- function(count_df){
  ...
  box_p <- long_count_df %>%
    ggplot(aes(x=sample,y=log(counts), fill = sample)) +
    geom_boxplot() +
