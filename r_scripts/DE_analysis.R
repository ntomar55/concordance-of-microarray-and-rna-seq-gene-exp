...
df_cleaner <- function(df){
  ...
  df <- df %>%
    ...
    select(genes, pvalue, log2FoldChange, treatment) %>%
    dplyr::slice_head(n=10)
  df$pvalue <- formatC(df$pvalue, digits = 4)
  df$log2FoldChange <- formatC(df$log2FoldChange, digits=3)
  return(df)
}
...
top10_table <- function(AhR, CAR, DNA){
  ...
  top_10 <- top_10 %>%
    ...
    pivot_wider(names_from = treatment,
                values_from = c(genes, pvalue, log2FoldChange)) %>%
    select(-row) %>%
    relocate(pvalue_AhR, .after = genes_AhR) %>%
    relocate(log2FoldChange_AhR, .after=pvalue_AhR) %>%
    relocate(pvalue_CAR_PXR, .after = genes_CAR_PXR) %>%
    relocate(log2FoldChange_CAR_PXR, .after = pvalue_CAR_PXR)
  
  table <- top_10 %>%
    gt() %>%
    # group columns
    tab_spanner(label = "AhR vs Control",
                columns = c(genes_AhR, pvalue_AhR, log2FoldChange_AhR)) %>% 
    tab_spanner(label = "CAR/PXR vs Control",
                columns = c(genes_CAR_PXR, pvalue_CAR_PXR, log2FoldChange_CAR_PXR)) %>%
    tab_spanner(label = "DNA Damage vs Control",
                columns = c(genes_DNA_Damage, pvalue_DNA_Damage, log2FoldChange_DNA_Damage)) %>%
    # color to easily distinguish
    tab_style(style = list(cell_fill(color = "lightgrey")),
              locations = cells_body(columns = c(genes_CAR_PXR, pvalue_CAR_PXR, log2FoldChange_CAR_PXR))) %>%
    # add footnote
    ...
    cols_label(genes_AhR = "Genes",
               pvalue_AhR = "P-Value",
               log2FoldChange_AhR = "Log2(FC)",
               genes_CAR_PXR = "Genes",
               pvalue_CAR_PXR = "P-Value",
               log2FoldChange_CAR_PXR = "Log2(FC)",
               genes_DNA_Damage = "Genes",
               pvalue_DNA_Damage = "P-Value",
               log2FoldChange_DNA_Damage = "Log2(FC)") %>%
    # add title and subtitle
    tab_header(title = "Top 10 Differentially Expressed Genes from DESeq2",
               subtitle = "Ranked by Significance (P-Value)") %>%
